...
- [ ] T003 [P] [US1] Add `TestLocalTrackEmbedMatch(_LocalTrackTestBase)` to `tests/tracks/test_local_track.py` with 9 test methods:
...
- [ ] T007 [P] [US2] Add `TestLocalTrackServiceRefCoexistence(_LocalTrackTestBase)` to `tests/tracks/test_local_track.py` with tests:
...
- [ ] T008 [P] [US3] Add `TestSpotifyTrackServiceContract` to `tests/tracks/test_spotify_track.py` with tests:
  - `test_permalink_returns_track_url`
  - `test_service_name_returns_spotify`
  - `test_spotify_track_is_service_track`
- [ ] T009 [P] [US3] Add `TestTrackContracts` to `tests/tracks/test_local_track.py` with tests:
...
- [ ] T010 [P] Run `uv run mypy .` and resolve any type errors introduced by new ABCs and overrides in `tracks/__init__.py`, `tracks/local_track.py`, `tracks/spotify_track.py`, and `matchers/spotify_matcher.py` (SC-005)
- [ ] T011 [P] Run `uv run ruff check . && uv run ruff format .` and resolve any linting or formatting issues (SC-006)
- [ ] T012 Run `uv run pytest tests/` and confirm the full test suite passes with no regressions (SC-003)
- [ ] T013 [P] Static verification: confirm `grep -rn "LocalTrack\|_normalize_isrc" matchers/` returns no matches (SC-001, SC-002)
