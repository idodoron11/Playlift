---
description: 'Python code review instructions for GitHub Copilot'
applyTo: '**'
...
Comprehensive code review guidelines for Python 3.11+ projects. These instructions follow best practices from prompt engineering and provide a structured approach to code quality, security, testing, and architecture review.
...
PREMIUM_DISCOUNT_RATE: float = 0.15
STANDARD_DISCOUNT_RATE: float = 0.10


def calculate_discount(order_total: float, item_price: float) -> float:
    is_premium_order = order_total > PREMIUM_THRESHOLD
    discount_rate = PREMIUM_DISCOUNT_RATE if is_premium_order else STANDARD_DISCOUNT_RATE
    return item_price * discount_rate
```
...
def test_calculate_discount_returns_10_percent_for_orders_under_threshold():
    order_total = 50.0
    item_price = 20.0

    discount = calculate_discount(order_total, item_price)

    assert discount == pytest.approx(2.00)
```
...
Add test case:
```python
def test_process_payment_processes_full_refund_when_order_is_cancelled():
    order = create_order(total=100.0, status="cancelled")

    result = process_payment(order, payment_type="refund")
...
if not user or not user.is_active or not user.has_permission("write"):
    return
# do something
```
````
...
### Code Quality
- [ ] Code follows PEP 8 and passes `ruff check` / `ruff format` without errors
- [ ] Names follow Python conventions: `snake_case` for functions/variables, `PascalCase` for classes, `UPPER_SNAKE_CASE` for constants
- [ ] All public functions, methods, and classes have Google-style docstrings
- [ ] All function signatures have complete type hints using modern syntax (`list[str]`, `X | None`)
- [ ] Functions/methods are small and focused (ideally < 30 lines)
- [ ] No code duplication
- [ ] Complex logic is broken into simpler parts
- [ ] Error handling is appropriate — specific exceptions, no bare `except:`
...
### Testing
- [ ] New code has appropriate test coverage (pytest)
- [ ] Test functions follow naming convention `test_<what>_<condition>_<expected>`
- [ ] Tests use Arrange-Act-Assert structure
- [ ] Tests cover edge cases and error scenarios
- [ ] Tests are independent and deterministic (no shared mutable state)
- [ ] External dependencies (I/O, network, filesystem) are mocked via `unittest.mock`
- [ ] No tests that always pass or are commented out

### Performance
- [ ] No obvious performance issues (N+1, memory leaks)
- [ ] Appropriate use of caching (e.g., `functools.lru_cache`)
- [ ] Generators used for large sequences instead of building full lists
- [ ] Context managers (`with`) used for resource management (files, connections)
...
When performing a code review, also verify::

1. **Type safety**
   - All public APIs have complete type hints
   - No use of `typing.List`, `typing.Dict`, `typing.Optional` — prefer built-in generics and `X | None`
   - `mypy` passes in strict mode

2. **Idiomatic Python**
   - f-strings used for string formatting — no `%`-formatting or `.format()`
   - `pathlib.Path` used for filesystem operations — no `os.path`
   - `enum.Enum` (or `enum.StrEnum`) used for named constants instead of bare string/int literals
   - Comprehensions preferred over `map`/`filter` with `lambda` for simple expressions
   - `@dataclass` or `NamedTuple` used for data-carrying classes instead of ad-hoc dicts

3. **Error handling**
   - Custom exception classes defined for domain errors — no bare `raise Exception(...)`
   - No bare `except:` — always catch specific exception types
   - Resources managed with `with` context managers
...
- **Build Tool**: `uv` (dependency management and virtualenv)
- **Linting / Formatting**: `ruff check .` / `ruff format .`
- **Type Checking**: `mypy .` (strict mode)
- **Testing**: `pytest`
- **Code Style**: PEP 8, Google-style docstrings